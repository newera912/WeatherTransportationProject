...
public class TestTravelTime {
	...
	public static void testSingleFileChangePoint(String singleFile,
	...
	public static void CaseStudy(int maxwin, int sss) {
		...
		List<String> caseDates = (List<String>) Arrays.asList("20160327");
		...
		for (String type : Arrays.asList("travelTime")) {// ,"wind"£¬"temp","temp9","press","wind","windDir","windMax","rh","rad")){
			...
			for (File apdmFile : new File(folder).listFiles()) {
				...
				if (methodType.equals("CP")) {
					...
					testSingleFileChangePoint(folder + apdmFile.getName(),
							outFile, maxwin, sss);
				} else {
					System.out.println("---CP3---");
					testSingleFileChangePoint2(folder + apdmFile.getName(),
							outFile, maxwin, sss);
			...
			String resultFilepath = sss
					+ "/debugI90trafficWAllYearEvent_TopK_result-"
					+ methodType
					+ "_baseMeanDiff_20_s_" + sss + "_wMax_" + maxwin
					+ "_filter_TIncld_0.7.txt";
			String resultFilepath2 = sss
					+ "/debugI90trafficWAllYearEvent_TopK_result-"
					+ methodType + "_baseMeanDiff_20_s_" + sss + "_wMax_"
					+ maxwin + "_filter_TIncld_0.7_Top.txt";
 ...
 + maxwin
			...
			String resultFilepathMulti = sss
					+ "/debugI90trafficWAllYearEvent_TopK_result-"
					+ methodType + "_baseMeanDiff_20_s_" + sss + "_wMax_"
					+ maxwin + "_filter_TIncld_0.7_Top_multi.txt";
			...
			try {
				...
				allWriterOutTop = new FileWriter("outputs/trafficData/" + type
						...
						false);
				singleWriterOutTop= new FileWriter("outputs/trafficData/" + type
						+ "_CaseStudy/" + methodType + "/" + resultFilepathSignle,
						false);
				multiWriterOutTop= new FileWriter("outputs/trafficData/" + type
						+ "_CaseStudy/" + methodType + "/" + resultFilepathMulti,
						false);
