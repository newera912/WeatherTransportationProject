...
public class MultiVarData {
	...
	public MultiVarData(String[] filePaths){
		int index=0;
		String type="";
		...
		for(String filePath:filePaths){
			...
			twGraphList.put(index,new TransWeatherRealGraph(apdmList.get(index),type,typeMap.get(type)));
			index++;
		}
		numberOfVariable=filePaths.length;
		System.out.println("number of Var "+numberOfVariable);
	}
