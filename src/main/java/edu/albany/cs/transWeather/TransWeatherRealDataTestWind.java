...
public class TransWeatherRealDataTestWind {
...
//				}
	...
	public static void CaseStudy(int maxwin,int sss){
		...
		String methodType="CP";
		for(String type:Arrays.asList("temp","temp9","press","wind","windDir","windMax","rh","rad")){//,"wind"£¬"temp","temp9","press","wind","windDir","windMax","rh","rad")){		
			...
			for(File apdmFile : new File(folder).listFiles()){
				...
				if(methodType.equals("CP")){
					...
					testSingleFileChangePoint2(folder+apdmFile.getName(), outFile,"outputs/mesonetPlots/"+type+"_CaseStudy/true_values3.txt",prf1File,maxwin,sss);
					
				}
				count++;
			}//file
			
			ArrayList<ResultItem> groundTrueItems=getGroundTruthRI("outputs/mesonetPlots/"+type+"_CaseStudy/true_values3.txt");
			ArrayList<ResultItem> resultItems=new ArrayList<ResultItem>();
			
			FileWriter allWriter = null;
			FileWriter allWriterOut = null;
			String prf1Filepath=sss+"/prf1/multiVAR_PRF1_result-"+methodType+"_baseMeanDiff_20_s_"+sss+"_wMax_"+maxwin+"_filter_TIncld_0.7.txt";
			String resultFilepath=sss+"/multiVAR_TopK_result-"+methodType+"_baseMeanDiff_20_s_"+sss+"_wMax_"+maxwin+"_filter_TIncld_0.7.txt";
			try {
