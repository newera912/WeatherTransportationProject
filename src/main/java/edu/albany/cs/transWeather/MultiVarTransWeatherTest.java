...
public class MultiVarTransWeatherTest {
	...
	public static Map<String,Double> threshold = new HashMap<String,Double>(){{
		put("temp",9.83);put("temp9",8.99);put("press",3.21);put("wind",10.64);
		put("windDir",554.22);put("windMax",18.31);put("rh",31.69);put("rad",765.65);
	}};
	...
	public static void testSingleFileChangePoint(String[] filePaths, String resultFileName,String gtFileName,String prf1File,int mwin,int sss) {
		...
		MultiVarTransWeatherTest mvTW=new MultiVarTransWeatherTest();
		...
		if (verboseLevel > 0) {
			for(Map.Entry<Integer,TransWeatherRealGraph> entry : mvData.twGraphList.entrySet()){
			System.out.println("####Date Type: " + entry.getValue().type +" "+entry.getValue().typeID);
			System.out.println("X: " + Arrays.toString(Arrays.copyOf(entry.getValue().x[0], 5)));
			System.out.println("mean: " + Arrays.toString(Arrays.copyOf(entry.getValue().mu, 5)));
			System.out.println("std: " + Arrays.toString(Arrays.copyOf(entry.getValue().sigma, 5)));			
		...
		if (verboseLevel > 0) {
			...
			System.out.println("X: " + Arrays.toString(Arrays.copyOf(mvNodes.get(0).X[0], 10)));
			System.out.println("Y: " + Arrays.toString(Arrays.copyOf(mvNodes.get(0).mean, 10)));
		
			
			System.out.println("X: " + Arrays.toString(Arrays.copyOf(mvNodes.get(1).X[0], 10)));
			System.out.println("Y: " + Arrays.toString(Arrays.copyOf(mvNodes.get(1).mean, 10)));
...
//				}
				...
				for(int q=0;q<N;q++){
					for(mvNode mvn:mvNodes){
						...
						for(int p=1;p<S.length-1;p++){	
							...
							tempResult=EMSScores(mvn.X[q],S[0],p,S[S.length-1]);					
						...
						tempDiff=Math.abs(T1Mean-mvn.hist_base[q])>=Math.abs(T2Mean-mvn.hist_base[q])?Math.abs(T1Mean-mvn.hist_base[q]):Math.abs(T2Mean-mvn.hist_base[q]);
				...
				//System.out.println("Before "+mvNodes.size()+" "+ changedVarList.length+" "+ArrayUtils.toString(changedVarList));
				
				
				ArrayList<mvNode> mvNodesChange=new ArrayList<mvNode>();
				for(int k=0;k<mvNodes.size();k++){
					if(Arrays.asList(ArrayUtils.toObject(changedVarList)).contains(mvNodes.get(k).typeID)){
				...
				for(int k=0;k<mvNodesChange.size();k++){
					mvNodesChange.get(k).func=new EMSStat(b,mvNodesChange.get(k).cV);
					
				}
				
				MultiVarGraphMP graphMP =new MultiVarGraphMP(mvNodesChange, s, g, B, t);
				//Utils.stop();
...
//				GraphMP graphMPWind = new GraphMP(graphWind, s, g, B, t, funcWind,cWind);
				...
				System.out.println("XXNorm:"+Arrays.toString(mvNodes.get(0).hist_base));
				...
				System.out.println("Var Index: "+ArrayUtils.toString(changedVarList));
	...
	public static int[] getChangeVarList(ArrayList<mvNode> mvNodes, double[] b,int sss) {
...
//				}
	...
	public static void CaseStudy(int maxwin,int sss,String month){
		...
		String[] var_types = { "temp", "temp9", "wind", "windMax", "rh", "rad" };
		
		//String folder="data/mesonet_data/trans/";
		boolean fileExistForAllVar=true;
		String fileName="";
		String[] filePath=null;
		for(File apdmFile : new File("data/mesonet_data/"+var_types[0]+"_APDM/").listFiles()){
			...
			for(int i=0;i<var_types.length;i++){
				filePath[i]="data/mesonet_data/"+var_types[i]+"_APDM/"+fileName;
		...
		String prf1Filepath = sss + "/prf1/AllYearEvent_multi_PRF1_result-"
				+ methodType + "_baseMeanDiff_20_s_"
				+ sss + "_wMax_" + maxwin + "_filter_TIncld_0.720160412.txt";
		...
		try {
			...
			allWriterOutTop = new FileWriter("outputs/mesonetPlots/multi_CaseStudy/"+methodType+"/"+resultFilepath2, false);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	    ...
	    for(int i=0;i<allResultList.size();i++)
	    {
	        ...
	        try {
				allWriterOut.write(allResultList.get(i).score+" "+ArrayUtils.toString(allResultList.get(i).changedVarList)+" "+ArrayUtils.toString(allResultList.get(i).Stations)+" "+ArrayUtils.toString(allResultList.get(i).timeSlots)+" "+allResultList.get(i).date+"\n");
