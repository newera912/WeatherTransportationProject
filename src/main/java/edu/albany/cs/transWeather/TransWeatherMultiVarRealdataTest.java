...
//				}
	...
	public static void testSingleFileChangePoint(String tempFile,String windFile, String resultFileName,String gtFileName,String prf1File,int mwin,int sss) {
		long startTime = System.nanoTime();
		FileWriter fileWriter=null;
		FileWriter prf1Writer=null;
		
		APDMInputFormat apdmTemp = new APDMInputFormat(tempFile);
		APDMInputFormat apdmWind = new APDMInputFormat(windFile);
		
		TransWeatherRealGraph graphTemp = new TransWeatherRealGraph(apdmTemp);
		TransWeatherRealGraph graphWind = new TransWeatherRealGraph(apdmWind);
		
		String[] paths=tempFile.split("/");
		String date=paths[paths.length-1].split("_")[0];
		try {
			prf1Writer = new FileWriter(prf1File, false);
			fileWriter = new FileWriter(resultFileName, false);
		...
		//TransWeatherGraph graph = new TransWeatherGraph(apdm,"grid");
		if (verboseLevel > 0) {
			System.out.println("X: " + Arrays.toString(Arrays.copyOf(graphTemp.x, 5)));
			System.out.println("mean: " + Arrays.toString(Arrays.copyOf(graphTemp.mu, 5)));
			System.out.println("std: " + Arrays.toString(Arrays.copyOf(graphTemp.sigma, 5)));			
			System.out.println("trueSubGraphSize: " + graphTemp.trueSubGraph.length);
			
			System.out.println("X: " + Arrays.toString(Arrays.copyOf(graphWind.x, 5)));
			System.out.println("mean: " + Arrays.toString(Arrays.copyOf(graphWind.mu, 5)));
			System.out.println("std: " + Arrays.toString(Arrays.copyOf(graphWind.sigma, 5)));			
			System.out.println("trueSubGraphSize: " + graphWind.trueSubGraph.length);
		...
		double[][] Xtemp = graphTemp.x;           //data Matrix, X_ij mean the ith station jth time slot value
		double[] meanTemp = graphTemp.mu;
		double[] cTemp=new double[Xtemp.length];  //input value (observed counts or values)
//		double[] bTemp= new double[Xtemp.length]; // base, all set to zero
//		Arrays.fill(bTemp, 1.0D);
		double[] hist_base_temp= new double[Xtemp.length];
		
		/*wind*/
		double[][] Xwind = graphWind.x;           //data Matrix, X_ij mean the ith station jth time slot value
		double[] meanWind = graphWind.mu;
		double[] cWind=new double[Xwind.length];  //input value (observed counts or values)
//		double[] bWind= new double[Xwind.length]; // base, all set to zero
//		Arrays.fill(bWind, 1.0D);
		double[] hist_base_wind= new double[Xwind.length];
		
		/*data parameters*/
		double[] c=new double[Xtemp.length];  //input value (observed counts or values)
		double[] b= new double[Xtemp.length]; // base, all set to zero
		Arrays.fill(b, 1.0D);
		...
		if (verboseLevel > 0) {
			...
			System.out.println("X: " + Arrays.toString(Arrays.copyOf(Xtemp[0], 10)));
			System.out.println("Y: " + Arrays.toString(Arrays.copyOf(meanTemp, 10)));
			for (int i : apdmTemp.data.trueSubGraphNodes) {
				System.out.print(Xtemp[i] + " ");
			}
			System.out.println();
			for (int i : apdmTemp.data.trueSubGraphNodes) {
				System.out.print(meanTemp[i] + " ");
			}
			
			System.out.println("X: " + Arrays.toString(Arrays.copyOf(Xwind[0], 10)));
			System.out.println("Y: " + Arrays.toString(Arrays.copyOf(meanWind, 10)));
			for (int i : apdmWind.data.trueSubGraphNodes) {
				System.out.print(Xtemp[i] + " ");
			}
			System.out.println();
			for (int i : apdmWind.data.trueSubGraphNodes) {
				System.out.print(meanWind[i] + " ");
			}
		...
		/* Generate all possible time window, window_size >=2 and less than maxWin*/
		for(int i=0;i<Xtemp[0].length-12;i++){
			for(int j=0;j<i+1;j++){
...
//				}
				...
				Arrays.fill(cTemp, 0.0D);
				Arrays.fill(hist_base_temp, 0.0D);
				
				Arrays.fill(cWind, 0.0D);
				Arrays.fill(hist_base_wind, 0.0D);
				 ...
				 * */
				for(int q=0;q<Xtemp.length;q++){					
					double tempTemp=0.0D;
					double tempWind=0.0D;
					
					for(int p=starIdx;p<S[0];p++){						
						tempTemp=tempTemp+Xtemp[q][p];
						tempWind=tempWind+Xwind[q][p];
					}
					if(S[0]==0){						
					hist_base_temp[q]=Xtemp[q][0];
					hist_base_wind[q]=Xwind[q][0];
					}else{
					hist_base_temp[q]=tempTemp/(S[0]-starIdx);
					hist_base_wind[q]=tempWind/(S[0]-starIdx);
				...
				/*Temp*/
				double emsScoreTemp=Double.NEGATIVE_INFINITY;
				Double[] tempResultTemp=new Double[4];				
				double tempDiffTemp=0.0D;
				
				double T1MeanTemp=0.0D;
				double T2MeanTemp=0.0D;
				
				/*wind*/
				double emsScoreWind=Double.NEGATIVE_INFINITY;
				Double[] tempResultWind=new Double[4];				
				double tempDiffWind=0.0D;
				
				double T1MeanWind=0.0D;
				double T2MeanWind=0.0D;
				
				for(int q=0;q<Xtemp.length;q++){	
					T1MeanTemp=0.0D;
					T2MeanTemp=0.0D;
					tempDiffTemp=0.0D;
					emsScoreTemp=Double.NEGATIVE_INFINITY;
					
					T1MeanWind=0.0D;
					T2MeanWind=0.0D;
					tempDiffWind=0.0D;
					emsScoreWind=Double.NEGATIVE_INFINITY;
					
					for(int p=1;p<S.length-1;p++){	
						
						tempResultTemp=EMSScores(Xtemp[q],S[0],p,S[S.length-1]);					
						
						if(tempResultTemp[0]>emsScoreTemp){
							index=tempResultTemp[1].intValue();
							//sIndex=temp[4].intValue();
							emsScoreTemp=tempResultTemp[0]; 
							//tempDiff=Math.abs(temp[2]-temp[3]);
							T1MeanTemp=tempResultTemp[2];
							T2MeanTemp=tempResultTemp[3];
														
						}
						
						tempResultWind=EMSScores(Xwind[q],S[0],p,S[S.length-1]);
						if(tempResultWind[0]>emsScoreWind){
							index=tempResultWind[1].intValue();
							//sIndex=temp[4].intValue();
							emsScoreWind=tempResultWind[0]; 
							//tempDiff=Math.abs(temp[2]-temp[3]);
							T1MeanWind=tempResultWind[2];
							T2MeanWind=tempResultWind[3];
					...
					//XX[q]=tempDiff;
					//System.out.println("Mean base diff:- "+q+" "+hist_base[q]+" "+T1Mean+" "+T2Mean+" "+Math.abs(T1Mean-hist_base[q])+" "+ Math.abs(T2Mean-hist_base[q])+" "+index+" "+ArrayUtils.toString(S));
					tempDiffTemp=Math.abs(T1MeanTemp-hist_base_temp[q])>=Math.abs(T2MeanTemp-hist_base_temp[q])?Math.abs(T1MeanTemp-hist_base_temp[q]):Math.abs(T2MeanTemp-hist_base_temp[q]);
					//System.out.println(tempDiff);
					cTemp[q]=tempDiffTemp;
					
					//System.out.println(">>"+Arrays.toString(temp)+Arrays.toString(S));
					//System.out.println("==>"+sIndex);
					//XX[q]=tempDiff;
					//System.out.println("Mean base diff:- "+q+" "+hist_base[q]+" "+T1Mean+" "+T2Mean+" "+Math.abs(T1Mean-hist_base[q])+" "+ Math.abs(T2Mean-hist_base[q])+" "+index+" "+ArrayUtils.toString(S));
					tempDiffWind=Math.abs(T1MeanWind-hist_base_wind[q])>=Math.abs(T2MeanWind-hist_base_wind[q])?Math.abs(T1MeanWind-hist_base_wind[q]):Math.abs(T2MeanWind-hist_base_wind[q]);
					//System.out.println(tempDiff);
					cWind[q]=tempDiffWind;
					
					c[q]=1.0D*cTemp[q]+1.0D*cWind[q];
					
//					if(cWind[q]>=5.0){
//						//System.out.println("wind change added..");
//						c[q]=1.0D*cTemp[q]+1.0D*cWind[q];
//					}else{
//						c[q]=1.0D*cTemp[q];//+1.0D*cWind[q];
//					}
				...
				GraphMP graphMP = new GraphMP(graphTemp, s, g, B, t, func,c);
//				
//				Function funcWind = new EMSStat(b,cWind);
//				GraphMP graphMPWind = new GraphMP(graphWind, s, g, B, t, funcWind,cWind);
				...
				System.out.println("XX:"+Arrays.toString(cTemp));
				System.out.println("XXNorm:"+Arrays.toString(hist_base_temp));
...
//		System.out.println("T2: "+T2.length+" "+Arrays.toString(T2)+" "+StatUtils.mean(T2)+" "+Math.sqrt(StatUtils.variance(T2)));
	 ...
	 * 
	\*********************************/
	public static void testSingleFileChangePoint2(String singleFile,String windFile, String resultFileName,String gtFileName,String prf1File,int mwin,int sss) {
		long startTime = System.nanoTime();
		FileWriter fileWriter=null;
		FileWriter prf1Writer=null;
		APDMInputFormat apdm = new APDMInputFormat(singleFile);
...
//				}
	...
	public static void CaseStudy(int maxwin,int sss){
		...
		String folderTemp="data/mesonet_data/temp_APDM/";
		String folderWind="data/mesonet_data/wind_APDM/";
		
		//String folder="data/mesonet_data/trans/";
		String fileName="";
		for(File apdmFile : new File(folderTemp).listFiles()){
			fileName=apdmFile.getName();			
			
			if (!new File(Paths.get("outputs/mesonetPlots/multi_CaseStudy/"+methodType+"/"+sss+"/").toString()).exists()) {
				new File(Paths.get("outputs/mesonetPlots/multi_CaseStudy/"+methodType+"/"+sss+"/").toString()).mkdirs();
				new File(Paths.get("outputs/mesonetPlots/multi_CaseStudy/"+methodType+"/"+sss+"/prf1/").toString()).mkdirs();
			}
			...
			String outFile="outputs/mesonetPlots/multi_CaseStudy/"+methodType+"/"+sss+"/"+apdmFile.getName();
			String prf1File="outputs/mesonetPlots/multi_CaseStudy/"+methodType+"/"+sss+"/prf1/"+fileName;
			if(methodType.equals("CP")){
				System.out.println("---CP---");
				testSingleFileChangePoint(folderTemp+fileName,folderWind+fileName, outFile,"outputs/mesonetPlots/multi_CaseStudy/true_values3.txt",prf1File,maxwin,sss);
			}else{
		...
		String prf1Filepath=sss+"/prf1/multi_PRF1_result-"+methodType+"_baseMeanDiff_20_s_"+sss+"_wMax_"+maxwin+"_filter_TIncld_0.7.txt";
		String resultFilepath=sss+"/multi_TopK_result-"+methodType+"_baseMeanDiff_20_s_"+sss+"_wMax_"+maxwin+"_filter_TIncld_0.7.txt";
		try {
			allWriter = new FileWriter("outputs/mesonetPlots/multi_CaseStudy/"+methodType+"/"+prf1Filepath, false);
			allWriterOut = new FileWriter("outputs/mesonetPlots/multi_CaseStudy/"+methodType+"/"+resultFilepath, false);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
...
//		}
		...
		System.out.println("outputs/mesonetPlots/multi_CaseStudy/"+prf1Filepath);
