...
public class TailApprox {
	...
	public TailApprox(ArrayList<Integer[]> edges, ArrayList<Double> edgeCostsW, double[] z, int s, int g, double B,
		...
		if (z == null) {
			...
			new IllegalAccessException(
					"Error in PCSFTail : vector z is null ...");
		}
		...
		if (verboseLevel >= 1) {
			...
			System.out.println("g : " + this.g + " s : " + s + " nu : "
					+ this.nu + " " + " C : " + this.C + " delta : "
					+ this.delta);
		}
		/**
	...
	private F run() {
		...
		if (verboseLevel >= 1) {
			System.out.println("pi_min : " + minPi + " lambda0 : " + lambda0
					+ " c(F) : " + forest.costF + " 2*C : " + 2.0D * C
					+ " pi(\bar{F}) : " + forest.piFBar);
		}
		/** check this further, == 0.0D */
		if ((forest.costF <= 2.0D * C)
				&& (forest.piFBar == 0.0D || forest.piFBar <= 1e-06D)) {
		...
		forest = this.PCSF_GW(edges, this.getCostsLambda(lambdaL),
				this.prizesPi, g);
		return forest;
	}
	...
	private boolean checkEqu8Valid(double cT, double[] b, double[] bS, double[] bSPrime) {
		...
		if (b == null || bS == null || bSPrime == null || b.length == 0
				|| bS.length == 0 || bSPrime.length == 0) {
			return flag;
		}
		for (int i = 0; i < b.length; i++) {
	...
	private F PCSF_GW(ArrayList<Integer[]> edges, ArrayList<Double> cLambda, ArrayList<Double> pi, int g) {
		...
		for (double p : pi) { // check prize valid
			if (p < 0.0D) {
				new IllegalAccessException(
						"the prize should not be negative ...");
				System.exit(0);
			}
		}
		...
		FastPCST pcstFast = new FastPCST(edges, pi, cLambda, FastPCST.kNoRoot,
				g, FastPCST.PruningMethod.kStrongPruning, -1);
		ArrayList<Integer> nodesInF = null;
		ArrayList<Integer> resultEdges = null;
		if (!pcstFast.run(resultEdges, nodesInF)) {
			new IllegalArgumentException(
					"Error : Algorithm returned false. There must be an error. \n");
			System.exit(0);
		} else {
			/** indices of edges. */
		...
		if (forest.gamma != g) {
			System.out
					.println("Error : the number of trees in forest does not match with g ...");
			System.exit(0);
		}
		return forest;
	}
	...
	public class F {
		...
		public F(ArrayList<Tree> trees) {
			...
			for (Tree tree : trees) {
				for (Integer node : tree.nodesInT) {
					...
					if (allnodes.add(node)) {
						...
						System.out
								.println("Error : duplicated nodes found in pruneForest ...");
						System.exit(0);
					}
				}
			}
			...
			if (this.gamma != this.trees.size()) {
				...
				System.out.println("gamma is " + this.gamma
						+ " is not equal to " + this.trees.size());
		...
		public int[] pruningTrees(double[] y, ArrayList<Integer> tailNodes) {
			...
			for (Tree tree : this.trees) {
				...
				for (int i : adj.keySet()) {
					System.out
							.println("nei " + i + " : " + adj.get(i).keySet());
				}
				while (true) {
					boolean flag = false;
