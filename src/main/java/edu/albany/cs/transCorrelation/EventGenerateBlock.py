...
def getWeatherEvent(fileName):
    ...
    with open(fileName,"r") as wF:
        for count,line in enumerate(wF.readlines()):
            ...
            for sta in stations:
                starttime=date+"%03d"%sta_slot
                endtime=date+"%03d"%end_slot
                #print sta,time
                weatherEvents.append((sta,starttime,endtime))
    ...
    with open(fileName,"r") as wF:
        for count,line in enumerate(wF.readlines()):
            ...
            for tmc in tmcs:
                ...
                endtime=date+"%03d"%end_slot
                #print sta,time
                trafficEvents.append((tmc,starttime,endtime))                
...
def main():
    ...
    trafficEventsE=getTrafficEvent(trafficFielName,0,30)
    ...
    trafficEventsE+=getTrafficEvent(trafficFielName,140,190)
    ...
    trafficEventsW=getTrafficEvent(trafficFielName,0,30)
    ...
    trafficEventsW+=getTrafficEvent(trafficFielName,160,210)
    ...
    with open("WholeYearWETevents_Blocks100_30.txt","w") as output:
        ...
        for event in trafficEventsE:
            output.write("1 "+str(round(tmcsE[event[0]][0]))+" "+str(round(tmcsE[event[0]][1]))+" "+str(event[1])+" "+str(event[2])+" "+str("2"+"%02d"%event[0])+"\n")
        for event in trafficEventsW:
            output.write("1 "+str(round(tmcsW[event[0]][0]))+" "+str(round(tmcsW[event[0]][1]))+" "+str(event[1])+" "+str(event[2])+" "+str("3"+"%02d"%event[0])+"\n")   
