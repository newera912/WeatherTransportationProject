...
def RIPLEY(weatherEvent,trafficEvent,pair_distance,r,timeThreshold,A):
    PI=math.pi
    Kr=0
    k=len(weatherEvent)
    l=len(trafficEvent)
...
def main():
    ite=500
    output=open("RIPLEYresult2.txt","a+")
    ...
    evetnFileName="F:/workspace/git/Graph-MP/data/events/March_events.txt"
    weatherEvent=[]
    trafficEvent=[]
...
#     wevAdjList=collections.defaultdict(list)
#     for wev in weatherEvent:
#         wevID="%d%d%d"%(wev[0],wev[1],wev[2])
#         for tev in trafficEvent:
#             pairDistID=str(wev[0])+"-"+str(tev[0])            
#             if pair_distance.has_key(pairDistID):
#                 wevAdjList[wevID].append((tev[0],tev[1],tev[2],pair_distance[pairDistID]))
#     
#             
    ...
    for timeThreshold in timeThresholds:
        ...
        for r in radius:
            testStatisticsScore=RIPLEY(weatherEvent,trafficEvent,pair_distance,r,timeThreshold,A)            
            ...
            for i in range(ite):
                ...
                score=RIPLEY(weatherEvent,trafficEvent,pair_distance,r,timeThreshold,A)
